...
@interface SCDropDownMenuView()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource>
...
/// 记录最近选中column
@property (assign, nonatomic) NSInteger currentSelectColumn;
/// 记录上一次的section选择，用于回显
@property (assign, nonatomic) NSInteger lastSelectSection;

/// 记录最近选中的sections
@property (strong, nonatomic) NSMutableArray *currentSelectSections;
/// 菜单背景layers
@property (strong, nonatomic) NSMutableArray *currentBgLayers;
/// 菜单titlelayers
@property (strong, nonatomic) NSMutableArray *currentTitleLayers;
/// 菜单分隔竖线separatorlayers
@property (strong, nonatomic) NSMutableArray *currentSeparatorLayers;
/// 菜单箭头layers
@property (strong, nonatomic) NSMutableArray *currentIndicatorLayers;
...
@implementation SCDropDownMenuView
...
- (void)initAttributes {
    ...
    self.itemTextSelectColor = kColorWithRGB(51, 51, 51);
    self.itemTextUnSelectColor = kColorWithRGB(51, 51, 51);
    self.cellTextSelectColor = kColorWithRGB(211, 0, 0);
    self.cellTextUnSelectColor = kColorWithRGB(51, 51, 51);
    self.separatorColor = kColorWithRGB(255, 255, 255);
...
- (UIView *)bottomLineView {
    if (!_bottomLineView) {
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height-(1.0/kScreenScale), self.frame.size.width, (0.7/kScreenScale))];
...
- (UIView *)backgroundView {
    if (!_backgroundView) {
        _backgroundView = [[UIView alloc] initWithFrame:CGRectMake(self.frame.origin.x, self.frame.origin.y, kScreenWidth, kScreenHeight)];
...
- (UICollectionView *)leftCollectionView {
    if (!_leftCollectionView) {
        ...
        [_leftCollectionView registerClass:[SCDropDownMenuCollectionViewCell class] forCellWithReuseIdentifier:@"SCDropDownMenuCollectionViewCell"];
    }
...
- (UICollectionView *)rightCollectionView {
    if (!_rightCollectionView) {
        ...
        [_rightCollectionView registerClass:[SCDropDownMenuCollectionViewCell class] forCellWithReuseIdentifier:@"SCDropDownMenuCollectionViewCell"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_delegate) {
        ...
        if (tableView == _leftTableView) {
            ...
            if (haveItems) {
                SCDropDownMenuStyle style = SCDropDownMenuStyleTableView;
                ...
                switch (style) {
                    case SCDropDownMenuStyleTableView: {
                ...
                if ([self.delegate respondsToSelector:@selector(menuView:selectIndex:)]) {
                    SCIndexPatch *index = [[SCIndexPatch alloc] initWithColumn:_currentSelectColumn section:indexPath.row row:-1];
            ...
            if ([self.delegate respondsToSelector:@selector(menuView:selectIndex:)]) {
                SCIndexPatch *index = [[SCIndexPatch alloc] initWithColumn:_currentSelectColumn section:_lastSelectSection row:indexPath.row];
                [self.delegate menuView:self selectIndex:index];
            }
        }
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    SCDropDownMenuCollectionViewCell *cell = [_leftCollectionView dequeueReusableCellWithReuseIdentifier:@"SCDropDownMenuCollectionViewCell" forIndexPath:indexPath];
...
/**使用代码选中列表中选项*/
- (void)selectedAtIndex:(SCIndexPatch *)indexPath {
    // 判断传入Index是否合法
...
- (void)selectDeafult {
    for (NSInteger i = 0; i < _firstArray.count; i++) {
        if ([self numberOfRowsInColumn:i section:0] > 0) {
            SCIndexPatch *index = [[SCIndexPatch alloc] initWithColumn:i section:0 row:0];
            [self selectedAtIndex:index];
        } else {
            SCIndexPatch *index = [[SCIndexPatch alloc] initWithColumn:i section:0 row:-1];
            [self selectedAtIndex:index];
        }
    }
}
...
- (void)showListViewWithHaveItems:(BOOL)haveItems {
    ...
    if (haveItems) {
        switch (style) {
            ...
            case SCDropDownMenuStyleCollectionView: {
            ...
            case SCDropDownMenuStyleCustom: {
                [self.leftCollectionView removeFromSuperview];
                [self.rightCollectionView removeFromSuperview];
                [self.leftTableView removeFromSuperview];
                [self.rightTableView removeFromSuperview];
